...
\frame{
	...
	\begin{itemize}
		...
		\item Výukové materiály jsou dostupné online: \item \href{https://is.muni.cz/www/moravec/c2062_anorganicka_chemie_ii/}{is.muni.cz/www/moravec/c2062\_anorganicka\_chemie\_ii/}
...
\frame{
	...
	\begin{itemize}
		\item Aktuální verze všech výukových materiálů je dostupná na adrese:
		\begin{itemize}
...
\frame{
	...
	\begin{columns}
		...
		\item Elektrická vodivost je způsobena přítomností volně se pohybujících valenčních elektronů, tzv. elektronového plynu.
		\begin{itemize}
		...
		\item Další důležitou vlastností kovů je \textit{kujnost a tažnost}.
	...
	\begin{column}{.35\textwidth}
		\begin{figure}
			\adjincludegraphics[width=\textwidth]{img/pasova-teorie.png}
		\end{figure}
	\end{column}
	\end{columns}
}
...
\frame{
	...
	\end{itemize}
	\begin{figure}
		\adjincludegraphics[width=0.45\textwidth]{img/Campo-iron-meteorite.jpg}
...
\frame{
	...
	\begin{itemize}
		...
		\end{enumerate}
		\item Příkladem jsou pájky, slitiny cínu s mědí. Jejich teplota tání se pohybuje okolo 183~$^\circ$C, zatímco teplota tání cínu je 232~$^\circ$C a olova 328~$^\circ$C.
...
\subsection{Elektronová konfigurace}
\frame{
	\frametitle{}
	\begin{itemize}
...
\frame{
	...
	\begin{itemize}
		...
		\item Druhou výjimkou je gadolinium s konfigurací 4f$^7$ 5d$^1$ 6s$^2$, tady se projevuje stabilita stavu 4f$^7$.
	...
	\begin{columns}
		\begin{column}{.6\textwidth}
			...
			\begin{itemize}
				...
				\item Při vzniku chelátového komplexu nahrazením monodentátních ligandů vícedentátními dochází ke výraznému zvýšení entropie systému ($+\Delta S$).
			\end{itemize}
			\vfill
		\end{column}
...
\frame{
	\frametitle{}
	\textbf{Magnetické vlastnosti látek}\footnote[frame]{\href{http://fyzika.jreichl.com/main.article/view/295-magneticke-vlastnosti-latek}{Magnetické vlastnosti látek}}
	\begin{itemize}
		\item \textit{Diamagnetismus} -- zeslabují vnější magnetické pole. Ideálními diamagnetiky jsou supravodiče, projevuje se to Meissner-Ochsenfeldovým efektem.
		\item \textit{Paramagnetismus} -- tyto materiály zesilují vnější magnetické pole. Spiny jsou uspořádány do domén, při působení magnetického pole se všechny dipóly orientují souhlasně. V nepřítomnosti magnetického pole je jejich orientace náhodná a navzájem se nulují.
		\item \textit{Ferromagnetismus} -- ferromagnetické materiály vykazují samovolnou magnetizaci. Jedná se o většinu permanentních magnetů.
		\item \textit{Antiferromagnetismus} -- orientace jednotlivých domén jsou opačné a navzájem se nulují.
		\item \textit{Ferrimagnetismus} -- podobně jako u antiferromagnetických materiálů jsou sousední domény orientovány opačně, ale jejich velikost není stejná, proto si materiál udržuje zbytkovou magnetizaci.
	\end{itemize}
}
...
\frame{
	...
	\begin{itemize}
		...
		\item Přesnou polohu těchto ostrovů je obtížné určit, každé nově objevené jádro pomáhá zpřesnit modely.
		\item Nová supertěžká jádra lze produkovat několika způsoby:\footnote[frame]{\href{http://hp.ujf.cas.cz/~wagner/popclan/jadra/supertezke_prehled_2016.htm}{Jak se produkují a studují supertěžké prvky}}
		\begin{enumerate}
			\item Ostřelováním těžkých jader intenzivním proudem neutronů, např.:
			\begin{itemize}
				\item \ce{^{238}U + n -> ^{239}U ->[23 min] ^{239}Np + $\beta^-$ ->[56 hod] ^{239}Pu + $\beta^-$}
			\end{itemize}
			\item Ostřelováním terče s obsahem těžkých, stabilních jader jiným těžkým jádrem.
			\begin{itemize}
				\item \ce{^{64}Ni + ^{209}Bi -> ^{272}Rg + n}
				\item \ce{^{70}Zn + ^{208}Pb -> ^{277}Cn + n}
			\end{itemize}
		\end{enumerate}
...
\frame{
	...
	\begin{itemize}
		...
		\item Mají také vyšší elektrický odpor ne feromagnetické látky.
